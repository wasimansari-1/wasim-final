...
/**
 * Real-Time Firestore Sync for Instant Live Socket Updates on Technician Devices
 */
export async function syncFirestoreCall(callId, data = {}, action = "upsert") {
  try {
    if (!admin.apps.length || !callId) return;
    const firestore = admin.firestore();
    const docRef = firestore.collection("forwarded_calls").doc(String(callId));
    ...
    for (const [k, v] of Object.entries(data)) {
      if (v !== undefined && v !== null) {
        if (v instanceof Date) {
          cleanData[k] = v.toISOString();
        } else if (typeof v === "object" && (v._bsontype === "ObjectID" || v.toHexString)) {
    ...
    await docRef.set(cleanData, { merge: true }).catch(() => {});
    console.log("⚡ Live Socket Event Synced to Firestore for Call:", String(callId));
  } catch (err) {
